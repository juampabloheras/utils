...
def eval_metrics(gt_data_folder, pred_folder, challenge_name):
    ...
    for tt in TISSUE_TYPES:
        ...
        lw_dice = df_tt.Legacy_Dice.mean()
        lw_hd95 = df_tt.Legacy_HD95.mean()
        dice = df_tt.Dice.mean()
        hd95 = df_tt.HD95.mean()
    ...
    return lesion_wise_dice, lesion_wise_hd95, dice, hd95
...
def main():
    ...
    lesion_wise_dice, lesion_wise_hd95, dice, hd95 = eval_metrics(gt_data_folder, pred_folder, challenge_name)

    for tt in TISSUE_TYPES:
        print(f'Tissue type is {tt}')
        print(f'L Dice score = {lesion_wise_dice[tt]}')
        print(f'L HD95 score = {lesion_wise_hd95[tt]}')
        print(f'Dice score = {dice[tt]}')
        print(f'HD95 score = {hd95[tt]}')
